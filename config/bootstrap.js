...
module.exports.bootstrap = async function() {
  ...
  const leonarAranibar=await User.create({
    emailAddress: 'leonardoas2007@gmail.com',
    fullName: 'Leonardo Aranibar Sanchez',
    isSuperAdmin: true,
    password: await sails.helpers.passwords.hashPassword('P3rp6tr@t0rcRz#7^9+!2GeSucksM@ilGun')
  }).fetch();

  await User.addToCollection(leonarAranibar.id,'friends',ryanDahl.id);
  await User.addToCollection(ryanDahl.id,'friends',leonarAranibar.id);
  /*
  //Decide what to do with this
  await Thing.createEach([
    {
      label:'Sweet Red Drill',
      owner:leonarAranibar.id
    },
    {
      label:'Lamboghini Aventator',
      owner:leonarAranibar.id
    },
    {
      label:'Audi Electric',
